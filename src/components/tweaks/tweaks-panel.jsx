...
// ── Inline styles (kept lightweight, scoped) ─────────────────────
const STYLE = `
    ...
    return (
      <button
        ...
        style={{
          ...
          background: "rgba(31,31,31,0.8)", color: "#fff",
          border: "none", cursor: "pointer",
          font: "700 12px/1 monospace",
        }}
        title="Tweaks (Ctrl+Shift+E)"
      >
        ⚙
      </button>
    );
...
export function TweakRadio({ label, value, options, onChange }) {
  ...
  if (!fitsAsSegments) {
    // Fallback to simple buttons if labels are long
    return (
          ...
          {options.map((o) => {
            ...
            return (
              <button key={v} type="button"
                      style={active ? { background: "rgba(255,255,255,.9)", boxShadow: "0 1px 2px rgba(0,0,0,.12)" } : {}}
